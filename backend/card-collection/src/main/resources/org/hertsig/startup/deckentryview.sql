CREATE OR REPLACE VIEW deckentryview AS (
  ...
  FROM deckrow
    ...
    LEFT JOIN card backcard ON card.id = backcard.doublefacefront
      LEFT JOIN latestprinting backprinting ON backprinting.cardid = backcard.id AND card.layout = 'double-faced'
